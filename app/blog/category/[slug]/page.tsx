...
export async function generateMetadata({
  ...
  if (!cat) return { title: "Category not found | EVSelect.in" };
  return {
    title: `${cat.label} — EV Blog & Guides`,
    description: cat.description,
    alternates: altsFor(`/blog/category/${cat.slug}`, "en"),
  };
}
        ...
        <section className="relative overflow-hidden border-b border-ev-border/40">
          ...
          <div className="mx-auto max-w-7xl px-4 pb-10 pt-12 sm:px-6 lg:px-8">
            <Breadcrumbs
              items={[
                { name: "Blog", path: "/blog" },
                { name: cat.label, path: `/blog/category/${cat.slug}` },
              ]}
              className="mb-6"
            />
