...
import { BatteryLifeHero, CapacityRetentionChart } from "@/components/blog/BlogImages";
import { POSTS } from "@/lib/blog-posts";
import { blogPostingSchema } from "@/lib/seo";

const post = POSTS.find((p) => p.slug === "ev-battery-life-india-weather")!;

export const metadata: Metadata = {
  title: "EV Battery Life: How to Make Your Battery Last 10+ Years in Indian Weather",
  description: "India's extreme heat, monsoon humidity, and stop-go traffic are tough on EV batteries. Learn science-backed strategies to maximise battery life.",
  alternates: { canonical: "https://www.evselect.in/blog/ev-battery-life-india-weather" },
  openGraph: { title: "EV Battery Life: Make It Last 10+ Years in Indian Weather", type: "article" },
};

const factors = [
  { icon: "🌡️", label: "Sustained High Ambient Temperature", color: "#f43f5e",
    body: "For every 10°C increase above 25°C, lithium-ion cells lose capacity approximately twice as fast. Cities like Nagpur or Jaisalmer regularly exceed 45°C — causing measurable, irreversible capacity loss within hours on an unshaded EV.",
  ...
  { icon: "⚡", label: "Poor Charging Habits", color: "#ffb800",
    body: "Charging behaviour accounts for an estimated 30–40% of long-term degradation. The two worst habits are charging to 100% every night (for NMC) and regularly depleting to 0%. Both cause accelerated cell stress.",
  ...
  { icon: "🔌", label: "Over-Reliance on DC Fast Charging", color: "#00e5ff",
    body: "DC fast charging pushes high current through cells rapidly, generating significant heat. Using a 50kW+ fast charger as your primary daily charger accelerates degradation by an estimated 10–15% over five years.",
    tips: ["Use DC fast charging for road trips and emergencies","Prefer AC home charging for daily top-ups","Wait 20–30 minutes after a long ride before plugging in"] },
  { icon: "💧", label: "Monsoon Humidity & Water Ingress", color: "#39ff14",
    body: "India's monsoon season introduces a challenge that European battery research rarely addresses. High ambient humidity (90%+) combined with temperature cycling causes moisture to condense inside poorly-sealed battery enclosures.",
      ...
      <main className="pt-16 min-h-screen bg-ev-bg">
        ...
        <div className="bg-ev-surface/20 border-b border-ev-border/30">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <span className="inline-flex items-center px-3 py-1 rounded-full text-[11px] font-mono border mb-4 bg-ev-green/10 border-ev-green/30 text-ev-green">Battery Tech</span>
          ...
          <div className="grid lg:grid-cols-[1fr_280px] gap-12">
            <article>
              <p className="text-ev-text/75 font-body text-base leading-relaxed mb-5">
                When you spend ₹1.5 lakh on an electric scooter or ₹15 lakh on an electric car, the battery is the single most valuable component. The good news: battery degradation is <strong className="text-ev-green">not inevitable</strong>. The habits you build in the first two years of ownership have a disproportionate impact on long-term capacity retention — and, in turn, on what your EV is worth when you sell it, as we explain in our guide to <Link href="/blog/ev-resale-value-battery-health-india" className="text-ev-green hover:text-ev-cyan underline underline-offset-2">EV resale value and battery health in India</Link>.
              </p>

              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 my-8">
                {[
                  { value: "20%", label: "Capacity Lost by Bad Habits", sub: "vs good habits, 3 years", color: "#f43f5e" },
                  { value: "80%", label: "Ideal Max Charge Level", sub: "NMC daily use", color: "#39ff14" },
                  { value: "2×",  label: "Faster Degradation", sub: "Per 10°C above 25°C", color: "#ffb800" },
                  { value: "3000+",label: "LFP Charge Cycles", sub: "vs ~1,500 for NMC", color: "#00e5ff" },
              ...
              <h2 className="font-display font-bold text-white text-2xl sm:text-3xl mt-14 mb-5">Know Your Battery Chemistry</h2>
              ...
              {/* Capacity Chart */}
              <h2 className="font-display font-bold text-white text-2xl sm:text-3xl mt-14 mb-4">LFP vs NMC Capacity Retention</h2>
              <div className="w-full aspect-[760/320] rounded-2xl overflow-hidden border border-ev-border my-6">
                <CapacityRetentionChart />
              </div>
              <p className="text-ev-text/55 text-xs font-body italic text-center mb-10">
                LFP batteries retain significantly more capacity over 100,000+ km in Indian conditions compared to NMC chemistry. Capacity loss also directly shrinks usable range — see how that plays out in our look at <Link href="/blog/real-world-range-vs-arai-india" className="text-ev-green hover:text-ev-cyan underline underline-offset-2 not-italic">real-world range versus ARAI claims in India</Link>.
              </p>
              ...
              <div className="space-y-6">
                {factors.map(({ icon, label, color, body, tips }) => (
              ...
              <div className="rounded-2xl p-5 my-8 border bg-ev-green/5 border-ev-green/25">
                ...
                <p className="text-ev-text/70 font-body text-sm leading-relaxed">
                  If you implement just one recommendation from this article, make it this: <strong className="text-white">set a charge limit of 80%</strong> for everyday use. On NMC batteries, this single change can reduce calendar degradation by an estimated 25–30% over five years. Pairing this with low-cost overnight AC charging keeps both your battery and your bills healthy — our <Link href="/blog/home-ev-charging-cost-india" className="text-ev-green hover:text-ev-cyan underline underline-offset-2">home EV charging cost guide</Link> breaks down the numbers.
                </p>
              </div>
              ...
              <div className="mt-8 p-5 bg-ev-surface/40 border border-ev-border/40 rounded-2xl">
                ...
                <p className="text-xs font-body text-ev-text/50 leading-relaxed">The information on EVSelect.in is for general informational purposes only. Always verify details directly with the manufacturer. EVSelect.in is independent and not affiliated with any vehicle manufacturer.</p>
              </div>
            </article>
            ...
            <aside className="hidden lg:flex flex-col gap-5 sticky top-24">
              <div className="bg-ev-card border border-ev-border rounded-2xl p-5">
                ...
                <ul className="space-y-2 text-xs font-body text-ev-muted">
                  {["Battery Chemistry: LFP vs NMC","LFP vs NMC Retention Chart","5 Biggest Enemies","10-Year Maintenance Plan","Warranty Tips"].map((item)=>(
